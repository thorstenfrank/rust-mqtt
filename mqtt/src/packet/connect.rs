...
/// 
/// let decoded = ConnectPacket::try_from(&encoded[..]).unwrap();
...
/// use mqtt::packet::ConnectPacket;
/// 
/// let packet = ConnectPacket::with_client_id_str("my-client-id").unwrap();
/// ```
/// 
/// # Binary Format
/// 
...
impl MqttControlPacket<'_> for ConnectPacket {
...
mod tests {
    ...
    #[test]
    fn encode() {
        let mut conn = ConnectPacket::with_client_id_str("ENCTST").unwrap();
    ...
    fn client_id_validation() {
        assert!(ConnectPacket::with_client_id_str("abcäÖŁ").is_err());
        assert!(ConnectPacket::with_client_id_str("abncjidLJKLÄSDU134").is_err());
        assert!(ConnectPacket::with_client_id_str("ClientIDIsTooLongSaysSpec").is_err());
        
        // not really sure about this rule, actually
        //assert!(ConnectPacket::new("no whitespace allowed".to_string()).is_err());
        assert!(ConnectPacket::with_client_id_str("perfectly_valid").is_ok());
    }

    #[test]
    fn user_properties() {
        let mut packet = ConnectPacket::with_client_id_str("user_properties_test").unwrap();
